...
- JOIN
    ...
    ```
        ...
        -- (4079, 20)
        -- 결과집합은 왼쪽 테이블의 데이터 수보다 작거나 같다!! (특징)
        ...
        -- 조인 기준 -> 구성 다음시간 체크
        -- 중요: car_order의 데이터개수를 초과하면 안됨, 동일 개수 유지 : left join
        SELECT A.*
            ,B.prod_cd
            ,B.quantity
            ,C.price
        ...
        - 수학
            ...
            - 기타 수학함수
                ...
                ```
                    -- 수학 계산
                    -- 참고
                    -- 데이터가 특정구간에 밀집해 있다면 => 데이터를 흐트러 놔야 한다
        ...
        - 일반
            ...
            - raw 데이터에서 데이터를 범위(band)를 지정하여 해당되는 데이터에 특정 컬럼값 부여하는 방식
                ...
                ```
                    ...
                    -- 고객 데이터를 기반으로 연령대별로 분류(군집(그룹)해라)
    ...
    - SQL
        ...
        - INSERT
            ...
            ```
                ...
                -- 중복된 데이터가 존재 -> KEY 오류 발생
                -- 이미 가입된 아이디가 있다? -> 아이디가 중복된다!!
                -- (*)가장 기본적 형태를 가장 많이 사용한다!!
                INSERT INTO users
                ( uid, upw, age, email, regdate) 
                VALUES ( 'guest', '1234', '25', 'a@a.com', NOW());

                -- uid에 대한 unique 처리가 안되어 있어서 통과된다!! => 테이블 수정
                -- uid unique 인텍스 처리 필요!!
                INSERT INTO users
                ( uid, upw, age, email, regdate) 
                ...
                -- users 테이블과 동일한 구조의 users_copy 생성후
                -- 아래 쿼리 실행!!
                INSERT INTO users_copy SELECT * FROM users;

        - UPDATE
            - 데이터 수정
            - 필수적으로 조건 부여!! -> 누락되면 사고 위험이 있음 (관리 개념,개발시)
            - 기존 내용(데이터) 업데이트
            - 작성한 글/회원 정보/... 수정하기!! <= 예시
            ```
                -- 회원 정보 수정
