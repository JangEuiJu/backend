...
//    문법 : 클래스명과 동일, 메소드 형식, 리턴타입 x, 객체를 생성 역활, 멤버변수 초기화
//    종류 : 기본생성자(default constructor), 생성자 재정의(overloading), 생략가능
...
class Person3 {
    ...
    public Person3(String name, int age) {
        // this => 클래스 내에서 자기자신을 가르키는 키워드
        this.name = name; // 멤버변수 = 매개변수;
        this.age = age;
    }
}
// 객체 지향 프로그램(OOP)의 3대 특징 : 상속성, 다형성, 은닉성
// 4. 데이터 은닉(객체 지향 프로그램의 3대 특징 중 한개), 클래스 내부 데이터는 외부에서 직접  접근 x
//    제공되는 메소드를 통해서 접근해라(설계 기법) -> 메소드 : getter, setter
//    데이터 종류가 너무 많다면 => get/setter 언제 다 만들지? => 라이브러리 => lombok(롬복)
...
public class ClassInfo {
    ...
    public static void main(String[] args) {
        // 클래스는 객체의 틀(청사진, ...) -> 메모리에 객체를 띠우는게 목표 -> 프로그램 작동
        // 객체 생성법(문법) => new 클래스명([ 매개변수 ]);

        // 객체 생성 -> 인텔리J 해석 -> 코드 가이드 : AI로 처리하는 패턴
        // Person p <- 참조형(타입) 변수!!, 클래스명 => 내가만든 참조형 타입이 된다
        // 참조형 변수 = 객체!!
        Person p = new Person();
